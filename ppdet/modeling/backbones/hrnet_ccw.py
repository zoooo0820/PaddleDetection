        ...
        for i in range(block_num):
            layers.append(
                ConditionalChannelWeightingBlock(
                    in_channels, freeze_norm=freeze_norm,
                    norm_decay=norm_decay))
    ...
    def __init__(self,
        ...
        self.conv1s = nn.LayerList([
            ConvNormLayer(
                ...
                freeze_norm=freeze_norm,
                stride=stride,
                act="relu") for num_channels in in_channels
        ])
        self.conv2s = nn.LayerList([
            ConvNormLayer(
                ...
                freeze_norm=freeze_norm,
                stride=1,
                act=None) for num_channels in in_channels
        ])
        ...
        self.spatial_weighting = nn.LayerList([
            SpatialWeightingModule(
                ...
                ratio=4,
                freeze_norm=freeze_norm,
                norm_decay=norm_decay) for num_channels in in_channels
        ])

    def forward(self, inputs):
        x = inputs
        x = self.cross_resolution_weighting(x)
        x = [conv1(s) for s, conv1 in zip(x, self.conv1s)]
        x = [conv2(s) for s, conv2 in zip(x, self.conv2s)]
        x = [sw(s) for s, sw in zip(x, self.spatial_weighting)]

        out = [paddle.add(s1, s2) for s1, s2 in zip(inputs, x)]
        out = [paddle.nn.functional.relu(s) for s in out]
        return out
...
@register
class HRNetSmallV2CCW(nn.Layer):
    """
    ...
    def __init__(self,
                 ...
                 norm_decay=0.,
                 return_idx=[0, 1, 2, 3]):
        super(HRNetSmallV2CCW, self).__init__()
        ...
        self.channels = {
            ...
            16: [[16, 32], [16, 32, 64], [16, 32, 64, 128]],
        }
